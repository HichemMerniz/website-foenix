      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            {t('our')} <span className="text-foenix-gradient">{t('services')}</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {services.map((service, index) => (
            <div
              key={index}
              className="group bg-white rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 hover:border-foenix-orange/20 transform hover:-translate-y-2"
            >
              {/* Service Icon */}
              ...
              <ul className="space-y-2">
                {(() => {
                  const f = t(`list.${index}.features`) as unknown;
                  if (Array.isArray(f)) {
                    return (f as string[]).map((feature, featureIndex) => (
